package com.carincho.springboot.webflux.client.app.handler;
...
import com.carincho.springboot.webflux.client.app.models.services.ProductoService;
...
public class ProductoHandler {
	...
	public Mono<ServerResponse>upload(ServerRequest request) {
		...
		return errorHandler(
				...
				.map(
						multipart -> multipart
						.toSingleValueMap().get("file")
						
						).cast(FilePart.class)
				.flatMap(file -> service.upload(file, id))
				.flatMap(p -> ServerResponse.created(URI.create("api/client/".concat(p.getId()))).contentType(APPLICATION_JSON)
						.bodyValue(p))
				);
